...
#pragma warning restore CS0219 // The variable 'flag' is assigned but its value is never used
        ...
        public void TrainChildren()
        {
            foreach (Person p in this.Persons)
            {
                ...
                if (p.Trainable && GameObject.Random((int)(40 / (IsPlayer(p.Father.BelongedFaction) ? 1 : Session.Current.Scenario.Parameters.AIExtraPerson) / Session.Parameters.DayInTurn)) == 0)
                {
